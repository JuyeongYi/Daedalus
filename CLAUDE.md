...
- **미설치 스킬 이름 원격 조회**(사용자 확정 2026-09-07 — 클론 대신 파일만
  확인): `model/plugin/remote_skills.py`가 **GitHub API로 `skills/` 디렉토리
  목록 한 번**만 받아 스킬 이름을 얻는다(디렉토리명이 곧 스킬 이름 — 로컬
  스캔과 같은 규약). 설명은 받지 않는다: 스킬마다 파일을 또 받으면 요청이
  N배이고 랩핑에 필요한 것은 이름뿐이다.
  **언제 인터넷 요청이 나가는가 — 사용자가 그 플러그인을 지목했을 때만이다.**
  카탈로그를 열거나 새로고침하는 것만으로는 절대 나가지 않는다(291개 일괄
  조회는 API 한도에 걸린다). 결과는 커밋 SHA까지 넣은 키로
  `~/.daedalus/cache/remote-skills/`에 캐시되므로 같은 버전은 재요청이 없고,
  버전이 바뀌면 자동으로 다시 받는다. **GitHub만 다룬다** — 다른 호스트는
  `None`을 돌려주고 "설치 후 확인"으로 안내한다(추측해 엉뚱한 목록을 보여주는
  것보다 모른다고 말하는 편이 낫다). 표면: 카탈로그 창 "스킬 목록 받아오기"
  버튼(이 창에서 요청이 나가는 유일한 지점) / MCP `fetch_plugin_skills(
  plugin_id, refresh=False)` — 설치된 플러그인은 로컬에서 읽어 요청이 없다.
  테스트는 `fetch_skill_names`를 몽키패치해 **호출 횟수까지** 센다(요청 시점이
  이 기능의 계약이다).
