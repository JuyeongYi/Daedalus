...
| `transfer_skill_reused` | 한 TransferSkill이 **2개 이상 전이**에 붙으면 **에러** (A11) — `no_duplicate_skill_ref`가 스킬 placement에 대해 말하는 것과 같은 결이다. 전이 스킬은 "이 갈래를 건널 때 할 일"이라 그 갈래에 고유하고, 한 정의를 여러 전이가 공유하면 도착 스킬의 진입 맥락과 출발 스킬의 다음 단계가 같은 이름을 서로 다른 뜻으로 가리키게 된다. 순회 범위는 프로젝트 그래프 + 각 스킬/에이전트 FSM(`_scan_transitions` 재귀 — sub_machine/Region 포함). 메시지에 붙은 위치를 전부 나열한다(어디를 고쳐야 하는지 알아야 한다) |
...
6-c. **전이 스킬(TransferSkill) 수행 지시 (A11)**: 전이에 `skill_ref`(TransferSkill)가 붙어 있으면 **출발 스킬의 "다음 단계"** 항목이 "전이 스킬 `X`(`desc`)의 지침을 수행한 뒤 …"로 시작한다(`_transfer_prefix`). 스킬 타깃·에이전트 위임·위임 인라인("위임 완료 후: …")의 후속 전이 전부 해당. 에이전트 `.md`의 "## 호출 계약" 항목에도 "(호출자가 전이 스킬 `X`의 지침을 수행하고 위임한다)"가 합류한다.
    **왜 출발 쪽인가:** 도착 스킬의 "진입 맥락"은 이미 "전이 스킬 X의 지침을 **수행한 상태다**"라고 전제하고 읽는데(그 문구는 그대로 유지), 정작 수행하라는 지시가 어디에도 없어 **아무도 전이 스킬을 실행하지 않는 구조**였다(A11 진단). 지시를 만드는 쪽은 그 갈래를 건너는 출발 스킬이다. transfer가 없으면 기존 문구 그대로 — 하위 호환.
