...
> 상태: **완료** (2026-08-17 — 9 WP 전부 master 머지, 최종 1,637 passed).
> 결정 확정: 직렬화 v2 승격 / delegation 삭제·로컬 스킬 승격 / RF-2 B안.
> 잔여 후보(후속 WP): serialize.py 1,437줄(_migrate_v1 집약으로 증가 —
> 위생 테스트 허용 목록 등재), markdown/editor.py 928줄(단일 클래스 —
> 믹스인 분할은 '이동만' 밖), scene.py 1,004줄(응집도 높아 보류 유지).
