...
| 결정 대기 — 제안 단계 | §2 reference 랩핑의 스킬 링크 예외 · statusLine 도구 · 외부 에이전트 노드 · `get_project` 축약 기본값 | 사용자 결정 |
...
- **`context: fork` 스킬은 실행 서브에이전트를 지정해야 한다** (사용자 요청 2026-09-13).
  CC에서 `context: fork`는 스킬 본문을 프롬프트로 새 서브에이전트를 띄우고, 어느 에이전트로
  띄울지는 `agent` 필드가 정한다(미지정이면 `general-purpose`). 지금 Daedalus는 두 필드가
  매트릭스에 따로 있을 뿐이라 **fork로 두고 agent를 비워도 아무도 짚지 않는다** — 설계자가
  의도한 에이전트 대신 범용 에이전트가 돈다. 필요한 것: ① 검증 규칙(fork인데 agent 미지정이면
  경고 — 기본값 위임이 의도일 수 있으니 에러는 과함) ② 에디터에서 `agent`를 프로젝트 에이전트
  목록에서 고르게(자유 입력이면 오타가 컴파일까지 흘러간다. 지금 `dangling_string_reference`가
  잡는 `ProceduralSkillConfig.agent`와 같은 값) ③ MCP 패리티. 랩핑 스킬의 실행 에이전트
  (2026-09-12)와 결이 같다 — "이 단계는 어느 컨텍스트에서 도는가"를 설계에서 못 박는 것.
