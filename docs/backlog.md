...
| 결정 대기 — 제안 단계 | §2 **fork 스킬 노드**(실측 완료·설계 미결) · reference 랩핑의 스킬 링크 예외 · statusLine 도구 · 외부 에이전트 노드 · `get_project` 축약 기본값 | 사용자 결정 |
...
### fork 스킬 노드 (설계 논의 중 — 2026-09-13, 결론 전)

**배경.** CC의 `context: fork` 스킬은 SKILL.md를 작업 지시로 삼아 서브에이전트를 띄운다. 어느
서브에이전트로 띄울지는 `agent` 필드가 정한다. 지금 Daedalus는 절차형·선언형 스킬에 `context`·
`agent` 필드가 따로 있을 뿐이라, fork로 두고 agent를 비우거나 틀려도 아무도 짚지 않는다.

**사용자가 확정한 것**
- fork 스킬은 **별도 노드 타입**(스킬 종류)으로 만든다. 색도 **기존과 겹치지 않는 별도 색**(제안:
  구리색 `#c07a3a` — 금색은 유저 진입점 테두리, 청록은 참조 포트가 쓴다).
- **나머지 스킬에서는 fork와 agent 지정이 불가능**해야 한다(필드 제거 — fork를 표현하는 길은 하나).

**실측 사실 (CC 2.1.268 실행 파일 + `claude -p` 실행, 기록 파일의 실제 값으로 판정)**

| 항목 | 사실 |
|------|------|
| 스키마 | SKILL.md에 `context: inline\|fork`, `agent`, `background`, `hooks`가 있다(`fallback`은 @internal) |
| `background` | fork 전용. **기본은 백그라운드** — 부른 쪽을 기다리게 하지 않고 끝나면 task notification으로 보고. `false`면 부른 쪽이 기다린다 |
| agent 해석 | `agent ?? "general-purpose"`를 **agentType 정확 일치**로 찾고, 없으면 **조용히 general-purpose로 실행**(디버그 로그 "not available as a fork base"만 남음) |
| 플러그인 에이전트 이름 | `agent: body`(이름만) → 못 찾음·범용으로 실행. `agent: 플러그인:body` → 정상 |
| LOCAL 에이전트 이름 | `.claude/agents/body.md`는 `agent: body`로 정상 |
| **model** | 스킬 `sonnet` vs 에이전트 `haiku` → **스킬이 이긴다** |
| **effort** | 값을 뒤바꿔 두 번(low/max) → **두 번 다 스킬 값**(1.7초 vs 125초로도 확인) |
| **tools** | 스킬 `allowed-tools: Bash` vs 에이전트 `tools: Read` → **에이전트가 이긴다**. 스킬 allowed-tools는 도구를 늘리지 못한다 |
| 전달 구조 | 에이전트 본문 → 서브에이전트의 **시스템 프롬프트**, 스킬 본문 → **작업 지시** |

즉 fork에서 에이전트가 실질적으로 정하는 것은 **장비**(도구·권한 모드·MCP·최대 턴·주입 스킬·훅)뿐이고,
할 일과 모델·effort는 스킬이 정한다. **에이전트 본문(직접 지시)의 몫은 거의 없다.**

**설계 후보 — 에이전트 쪽을 어떻게 담을까 (미결)**
- **안 1. 노드가 몸을 소유.** fork 노드 안에 스킬 칸 + 에이전트 칸을 두고 함께 편집한다. 그래프의 다른
  에이전트를 고르지 않으므로 **보이지 않는 연결("점프")이 없다.** 대신 같은 장비를 여러 fork가 재사용할 수
  없다.
- **안 2. 실행 환경 라이브러리.** 장비만 담은 "실행 환경"을 훅 라이브러리처럼 목록으로 정의하고(캔버스에
  놓지 않음, 본문 없음) fork 스킬이 이름으로 고른다. 내장 `general-purpose`/`Explore`/`Plan`은 기본
  항목. 행동이 없는 장비라 공유해도 "멀리서 동작이 바뀌는" 일이 없고 재사용이 산다. **그래프에 배치된
  에이전트 노드는 고를 수 없다**(그것은 자기 절차를 가진 워크플로 단계라 겸하면 점프가 된다). 산출은
  실행 환경 하나 = `agents/<이름>.md` 하나(본문 없음) — 에이전트 컴포넌트와 이름 충돌 금지 규칙 필요.
- 논의는 안 2 쪽으로 기울었으나 **확정 전**이다.

**컴파일 제안 (배치된 fork 스킬)**
- 산출 SKILL.md: `context: fork` + `agent: <이름>`(**빌드 타깃별로 컴파일러가 결정** — 마켓 `플러그인:이름`,
  LOCAL `이름`) + `background: false`(부른 쪽이 기다려야 분기 가능) + 모델·effort.
- 분기는 **스킬 쪽**: "## Next Steps"(직접 인보크) 대신 **출구 + 갈래별 다음 행동을 보고하라**. 실제로 다음
  스킬을 부르는 것은 메인이다 — 서브에이전트가 부르면 워크플로가 서브에이전트 안에서 이어진다. 사용자가
  `/스킬`로 직접 시작해 앞 단계가 없어도 다음 행동이 보고에 실려 끊기지 않는다.
- 진행 기록은 에이전트 노드와 같이 **부르는 쪽(메인)**이 적는다. fork 쪽은 건드리지 않는다.
- 검증: fork 스킬도 **에이전트 1계층**으로 센다(`agent_chain_too_deep`/`agent_calls_higher_model` 합류).
  없는 실행 환경 이름은 **에러**(런타임이 조용히 범용으로 떨어지므로).

**열린 질문 (fable 검토 대상)**
1. 안 1 vs 안 2 — 재사용과 국소성 중 무엇을 택하나. 안 2의 "장비는 행동이 없으니 점프가 아니다"가 성립하나.
2. 에이전트 본문(시스템 프롬프트)을 아예 없앨지, 선택 칸으로 둘지.
3. 장비가 기본값뿐이면 에이전트 파일 없이 `agent: general-purpose`로 낼지.
4. 에이전트 노드(호출 포트)와 fork 스킬 노드의 경계 — 사용자에게 언제 어느 쪽을 쓰라고 안내하나
   ("지시가 몸에 있으면 에이전트 노드, 지시가 스킬에 있으면 fork").
5. 랩핑 스킬의 실행 에이전트(2026-09-12, 위임 방식)를 fork 방식으로 합칠지.

**아직 실측 안 한 것**
- 본문이 빈 에이전트 파일을 fork 몸으로 정상 사용하는지.
- fork에서 스킬 `allowed-tools`가 권한 사전 승인으로는 작동하는지.
