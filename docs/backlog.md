...
| 결정 대기 — 제안 단계 | §2 **fork 스킬**(설계안 완성·사용자 확정 대기·실측 4건) · reference 랩핑의 스킬 링크 예외 · statusLine 도구 · 외부 에이전트 노드 · `get_project` 축약 기본값 | 사용자 결정 |
...
**구현 전 실측**
1. fork 스킬에 model이 **없을 때** 에이전트의 model이 적용되는지(`fork_model_overrides_agent` 조건이 달라진다).
2. 내장 에이전트의 정확한 agentType 문자열(`Explore`/`Plan` 대소문자 — 정확 일치라 틀리면 조용히 범용).
3. 몸 에이전트의 `maxTurns`/`isolation`/`skills` 프론트매터가 fork에 적용되는지.
4. `/스킬 인자`의 `$ARGUMENTS`가 서브에이전트 지시에 치환되는지.
