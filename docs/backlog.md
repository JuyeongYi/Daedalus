...
### fork 스킬 (설계안 — 사용자 최종 확정 대기, 2026-09-13)

**배경.** CC의 `context: fork` 스킬은 SKILL.md를 작업 지시로 삼아 서브에이전트를 띄우고, 어느 서브에이전트로
띄울지는 `agent` 필드가 정한다. 지금 Daedalus는 절차형·선언형에 `context`·`agent` 필드가 따로 있을 뿐이라
fork로 두고 agent를 비우거나 틀려도 아무도 짚지 않는다 — 그러면 **조용히 범용 에이전트로 돈다**.

**사용자 확정 (2026-09-13)**
- fork 스킬은 **별도 스킬 종류(kind)** + **별도 색**(제안: 구리색 `#c07a3a`). 나머지 스킬은 fork·agent 지정 불가.
- fork 스킬이 고를 수 있는 에이전트는 **세 종류**: ① 내장(`general-purpose`/`Explore`/`Plan`) ② 사용 선언한
  외부 플러그인의 에이전트 ③ **캔버스에 배치되지 않은** 프로젝트 에이전트. 배치된 에이전트는 워크플로 단계라
  몸으로 겸하면 캔버스에 안 보이는 연결("점프")이 생기므로 제외한다.

**실측 사실 (CC 2.1.268 — 실행 파일 + `claude -p` 실행 후 기록 파일의 실제 값으로 판정)**
...
| agent 해석 | agentType **정확 일치**. 없으면 **조용히 general-purpose로 실행**(디버그 로그만) |
| 이름 형식 | 플러그인 에이전트는 `플러그인:이름`만 찾힌다. LOCAL(`.claude/agents`)은 `이름`으로 된다 |
| model·effort | **스킬이 이긴다**(effort는 값을 뒤바꿔 두 번 확인) |
| tools | **에이전트가 이긴다** — 스킬 `allowed-tools`는 도구를 늘리지 못한다 |
| 전달 구조 | 에이전트 본문 → 시스템 프롬프트, 스킬 본문 → 작업 지시 |

**설계안 (fable 검토 2회 반영 — 확정 대기)**
- **노드 모양:** 에이전트를 **소유하지 않는 가벼운 kind**. 새 필드는 `agent`(세 종류 참조) 하나이고, 필드
  매트릭스에서 `allowed_tools`를 뺀다(fork에서 효과가 없다). 피커에는 세 종류만 뜬다 — 배치된 에이전트는 아예 안 뜬다.
- **전환:** 절차형 ↔ fork는 랩핑 용도 전환처럼 **명시 액션 1 undo**. 이름·본문·설명·포트·전이 보존, fork로 갈 때
  `allowed_tools`는 버리고 그 사실을 보고한다.
- **본문:** fork 스킬 본문 = 작업 지시. 시스템 프롬프트는 참조된 에이전트의 것이다 — 따로 입력할 칸이 없다.
- **산출:** SKILL.md에 `context: fork` · `background: false`(부른 쪽이 보고를 기다려야 분기 가능) · 모델·effort ·
  `agent:`. 기본값은 `agent: general-purpose`를 **명시 배출**(결정적·읽는 사람에게 분명). 프로젝트 에이전트는
  마켓 `플러그인:이름` / LOCAL `이름`으로 컴파일러가 정하고, 내장·외부는 그대로 적는다. **컴파일러가 에이전트
  파일을 새로 만드는 일은 없다.** 몸으로 쓰이는 프로젝트 에이전트의 `.md`는 지금처럼 나가고, "## Invocation
  Contract"에 "Execution base of fork skill `X`"를 유도해 넣는다.
- **분기와 진행 기록:** fork 서브에이전트는 다음 스킬을 부르지도, 진행 기록을 쓰지도 않는다(몸이 `Explore`/`Plan`이면
  상태 파일 쓰기가 어색하고, 메인에는 지시가 없다). 대신 **보고가 지시가 된다** — 첫 줄 `EXIT: <출구> / NEXT: /<스킬>`,
  꼬리에 메인이 실행할 `daedalus-bb progress …` 명령 한 줄. 사용자가 `/스킬`로 직접 시작해도 끊기지 않는다.
- **자동 단락 정리:** 작업 재개·진입 맥락·다음 단계·블랙보드 단락은 메인 전제로 쓰였으므로 fork용 문구로 다듬는다.

**검증 규칙 (안)**

| 규칙 | 등급 | 내용 |
|------|------|------|
| `fork_agent_placed` | 에러 | 몸으로 쓰는 에이전트를 캔버스에 배치함. 드롭 자체는 허용하고 규칙으로 짚는다(조작 거절은 이유가 안 보인다) |
| `fork_agent_missing` | 에러 | 몸 에이전트가 없음(삭제·오타). 삭제 시 참조는 몰래 지우지 않고 보고한다 |
| (외부 에이전트 미선언) | 에러 | 사용 선언 안 된 플러그인의 에이전트 — 런타임이 조용히 범용으로 떨어진다(랩핑은 경고지만 fork는 에러) |
| `fork_model_overrides_agent` | 경고 | 스킬과 에이전트 양쪽에 model/effort가 있고 다름 — 에이전트 값은 무시된다. 편집기에도 "도구는 에이전트가, 모델은 이 스킬이 정한다" 한 줄 |
| `fork_agent_isolation_ignored` | 경고 | 몸 에이전트에 `isolation`이 있음 — fork에서는 적용되지 않는다(실측) |
| 깊이·모델 티어 | 에러 | fork 스킬을 **에이전트 1계층**으로 센다. fork의 `call_agents`도 셈에 포함 |

**선행 작업:** 외부 플러그인 카탈로그가 지금은 `skills/*/SKILL.md`만 훑는다 → `agents/*.md` 스캔 추가.

**보류:** 랩핑 스킬 실행 에이전트를 fork 방식으로 합치기 — 실행 에이전트는 생성물이라 세 종류에 들지 않고,
2026-09-12 사용자 확정(model/effort를 실행 에이전트로)과 부딪힌다. fork 도그푸딩 뒤 다시 본다.

**추가 실측 (2026-09-13, 2차 — 같은 방법)**

| 확인 | 결과 |
|------|------|
| 스킬에 model·effort가 **없을 때** | **에이전트 값이 적용된다**(haiku / sonnet+low로 확인) — 스킬에 값이 있을 때만 스킬이 이긴다 |
| 내장 에이전트 이름 | 정확한 문자열은 `general-purpose` / `Explore` / `Plan`(`statusline-setup`도 있음). 정확 일치라 대소문자까지 맞아야 한다. `agent: Explore`는 정상 실행(Edit·Write 없는 도구 목록, 메인 모델 상속) |
| 몸 에이전트 `skills:` 프리로드 | **적용된다**(도구로 파일을 못 읽게 막고 넣어 둔 비밀 토큰을 보고) |
| 몸 에이전트 `maxTurns` | **적용된다**(디버그 로그 "Reached max turns limit (1)") |
| 몸 에이전트 `isolation: worktree` | **적용되지 않는다** — 서브에이전트가 원래 폴더에서 돌았고 worktree가 생기지 않았다 |
| `/스킬 인자` | `$ARGUMENTS`가 서브에이전트 지시에 **치환된다** |
| 도구가 0개인 몸 | 이 환경에 없는 도구만 준 에이전트로는 fork가 **아무 일도 하지 못했다**(서브에이전트 기록이 남지 않음. 거부 로그는 못 찾음) |

**실측이 설계에 주는 변화**
- `fork_model_overrides_agent` 조건 확정: 스킬과 에이전트 **양쪽에 값이 있고 다를 때만** 경고. 스킬이 비면 에이전트 값이
  쓰이므로 정상이다. 편집기 안내: "비워 두면 몸 에이전트의 모델·effort를 쓴다".
- 몸 에이전트의 `isolation`은 fork에서 무시된다 → 경고 규칙 추가(아래 표).
...
- **외부 플러그인 에이전트 노드** — 감쌀 수 없으므로 입력만 있는 노드(에이전트 호출 포트로만 진입, 나가는 전이
