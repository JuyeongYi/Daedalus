...
- **에이전트 프론트매터 훅은 별개 경로다** — LOCAL 빌드에서 에이전트가
  ...
  **전역 배출 ∪ 산출 파일을 내는 컴포넌트의 참조**(`emits_output_file`)로 내므로, 꺼둔 훅을
  컴포넌트가 써도 없는 파일을 가리키지 않는다.
...
- **스킬 훅 (2026-09-13 실측으로 정정)** — 스킬도 `config.hooks`로 훅을 참조하고, 그 훅은
  **스킬이 활성인 동안만** 걸린다(SKILL.md 스키마: "Hooks registered while this skill is active.
  ...
  - 스크립트 파일은 `hooks_needing_scripts`가 스킬 참조를 **두 타깃 모두** 포함해 낸다. 세는
    집합은 `emit.common.emits_output_file` 하나가 정한다(원칙 1) — 참조 용도·비활성 랩퍼처럼
    SKILL.md를 내지 않는 스킬은 세지 않고, **SKILL.md를 내는 스킬은 종류를 가리지 않고** 센다.
    참조 스킬(`SKILL_FIELD_MATRIX` HOOKS = D)도 자기 SKILL.md에 `hooks:` 블록을 싣는다 —
    2026-09-19 이전에는 `is_reference_usage` 게이트가 참조 스킬을 통째로 건너뛰어 **없는
    스크립트를 가리키는 훅**이 산출됐다(D6).
...
- **UI**: `editors/hook_panel.HookLibraryPanel` — **상주 탭(인덱스 2)**. 모달 다이얼로그(`hook_editor.HookLibraryDialog`)는 3단 구조를 담을 수 없어 제거됐다(도구 메뉴 항목도 함께 — 탭이 늘 보이므로 지름길이 중복이다). 좌: 훅 목록(핸들러 없으면 ⚠). 우: 이벤트 콤보(matcher 미지원/미문서화를 문구에 표시) + matcher(받지 않는 이벤트면 잠금 + 이유 표시) + 핸들러 목록·폼(`_HandlerForm` — 타입이 바뀌면 통째로 다시 만든다. WP-11에서 `editors/hook_handler_form.py`로 분리했고, 타입별 **필드 구성**·**저장**은 `build_type_fields`/`save_type_fields` 두 `singledispatch`다 — 종전에는 같은 다섯 분기가 두 벌이라 한쪽만 고치면 "폼에는 보이는데 저장되지 않는" 조용한 실패가 됐다. 등록 집합이 `HOOK_HANDLER_TYPES`와 같은지는 `tests/view/editors/test_hook_handler_form.py`가 양방향으로 고정한다. 폴백은 공통 필드만 다루고 터지지 않는다 — 모르는 종류가 든 프로젝트도 열려야 한다). **"서브에이전트 프론트매터로 복사" / "hooks.json으로 복사"** 버튼이 이 프로젝트 밖의 파일에 붙여넣을 텍스트를 클립보드에 넣는다. `widgets/tag_input`의 `set_hook_name_provider`로 컴포넌트의 HOOKS TagInput이 훅 이름을 후보로 표시한다(A1 이후 **전역 훅 이름도 포함** — `app.set_project`가 `self.resolved_hooks()`를 등록). 전역 훅 표시는 아래 "전역 훅 2단 스코프 (A1)" 참조.
