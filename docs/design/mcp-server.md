...
- **계층:** MCP 도구는 GUI 어댑터라 `daedalus.view`를 임포트한다 — 편집은
  `view/actions/*`·`view/commands/*` 공유 액션을, 본문은 `view/editors/body_documents`의
  문서 레지스트리를 거쳐야 사용자의 undo 스택에 정확히 오른다(의도된 공유 실체).
  **다만 GUI 다이얼로그 모듈(`*_dialog`)은 임포트하지 않는다** — 창 파일에서 함수를
  빌려 오면 모델 질문의 실체가 창에 눌러앉고(원칙 1), 헤드리스 MCP 경로가 QDialog
  정의를 끌어온다. 실체는 모델(또는 actions)에 두고 창이 그것을 부른다.
  게이트: `tests/test_import_contracts.py::test_mcp_does_not_import_gui_dialog_modules`.
...
- **미리보기에는 산출 게이트가 없다.** 이번 빌드에 파일이 나가지 않는 컴포넌트도
  ...
  흔적은 부르는 쪽의 위임 지시에 나타난다 — 그 컴포넌트를 미리보라")을 말하는 `ValueError`가
  난다(원칙 5). 게이트는 `preview_component` 안에 있다 — GUI 진입점 셋은 `can_preview()`로 메뉴를
  흐리지만 MCP에는 흐릴 메뉴가 없어, 게이트가 호출자 쪽에만 있으면 MCP만 "emitter 등록이 없다"는
  **내부 사정**을 거절 이유로 말하게 된다(WP-9 리뷰 반영 — 원칙 2는 두 표면이 같은 이유를
  말하라는 뜻이기도 하다).
