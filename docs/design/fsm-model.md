...
- **상태·전략 서술의 디스패치 (WP-11, 2026-09-19 — 동작 불변, 골든 바이트 동일):** 컴파일러의 FSM 산문은 `isinstance` 사다리 셋이었다(상태 종류 7분기 ×2벌 · 평가 전략 5분기 · 트리거 1분기). 지금은 `compiler/emit/sections.py`의 `singledispatch` 넷이다 — `_describe_step`(상태) · `_describe_evaluation`(전략) · `_describe_trigger`(이벤트) · `_unguarded_is_else`(ChoiceState의 else 관례). **레지스트리가 아니라 singledispatch인 이유**는 기저 폴백이 옳기 때문이다: 모르는 상태는 이름만 말하고(`.`), 모르는 전략은 `"condition"`이다 — 구버전·미래 종류 하나가 컴파일 전체를 죽이지 않는다(`tests/compiler/test_state_prose_dispatch.py`가 폴백을 고정한다). 에이전트 legacy 산출(`agent_sections._describe_legacy_step`)은 **합치지 않았다** — Parallel/Choice/Terminate 분기가 없고 CompositeState 문구에 "(runs in its own context)"가 없으며 ExitPoint에 `exit` 표지를 붙인다. 합치면 구버전 프로젝트의 산출 바이트가 바뀐다.
- **의사 상태의 `kind`는 상수로 선언한다** (WP-11): `pseudo.ENTRY_POINT_KIND`/`EXIT_POINT_KIND`. 뷰가 "이 상태가 진입/출구 표지인가"를 묻던 `isinstance` 5건(`node_item` 4 · `graph_io` 1)이 이 값을 키로 쓰는 표 조회 한 번이 됐다 — 뷰가 모델 클래스를 직접 수입하지 않는다. ExitPoint만 색을 모델이 들고 있어 그 한 가지는 `node_item._pseudo_border` singledispatch(폴백 = 표의 색)다.
- **identity 동등성 + hashable 회귀망** (WP-11): `tests/model/fsm/test_identity_equality_policy.py`가 `State`의 **구체 서브클래스 전수**(pkgutil 발견)와 `StateMachine`/`Region`/`Section`/`Transition`에 대해 ① 동명 인스턴스가 다르다 ② hashable이다 ③ `__eq__`/`__hash__`가 `object`의 것 그대로다를 본다. 서브클래스에 `@dataclass`를 재선언하며 `eq=False`를 빠뜨리면 순회의 `seen` 집합이 `TypeError`로 죽거나 동명 상태 둘이 접힌다 — 둘 다 조용한 회귀라 전수로 감시한다.
- **`SimpleState.skill_ref`의 타입은 `PluginComponent | None`이다**(WP-2b, Q35) — 종류 유니온이 아니다. 예전 `StepSkill | DeclarativeSkill | AgentDefinition`은 배치 가능한 종류가 빠져 있어도 아무것도 실패하지 않는 낡은 선언 표였다(스멜 ⑤). 실체 판정은 `c.effective_placement() is PlacementRole.STATE`이고, 새 종류는 `fsm/**`를 한 줄도 건드리지 않는다 — `tests/model/fsm/test_state_is_kind_neutral.py`가 소스에 구체 컴포넌트 클래스 이름이 없음을 고정한다.
- **그래프 노드가 될 수 있는 컴포넌트**는 `model/plugin/placement.is_state_placeable`가 정한다(WP-FK2) — `StepSkill`(절차형·fork 2종), `AgentDefinition`, `ExternalAgent`. `ForkAgent`는 fsm·포트가 없고 fork 스킬이 부르는 실행 기반이라 **노드가 될 수 없다**. 캔버스·MCP·"여기에 만들기"가 모두 같은 함수를 부른다 — 상세는 `plugin-model.md` "배치 가능 판정".
