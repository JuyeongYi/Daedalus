...
| `dangling_string_reference` | `AgentConfigBase.skills`(에이전트 두 종류 모두), `PluginProject.reference_placements.skill_name`의 문자열 참조 실존 검사 (스킬 이름은 전역 skills 기준). fork 스킬 `agent`는 내장·외부도 가리킬 수 있어 여기서 보지 않는다 — `fork_agent_missing`이 맡는다. **외부 플러그인 스킬 참조(`플러그인:스킬`, WP-B)는 여기서 보지 않는다** — `AgentConfigBase.name_refs(Bucket.SKILLS)`가 콜론 있는 참조를 제외하므로(`is_external_skill_ref`) 이 규칙은 프로젝트 스킬 이름만 본다. 그 참조의 배선 여부는 `undeclared_external_plugin`이 맡는다 |
...
| `external_source_missing` | **외부 정본을 선언한 컴포넌트**의 source 빈 값·형식 불일치 경고 (`플러그인[@마켓]:이름`). 대상은 `c.external_source is not None`인 것 전부다 — WP-2b에서 랩핑 전용 `wrapped_source_missing`을 **개명·일반화**했다(Q34). 종류를 묻지 않으므로 외부 정본을 갖는 새 종류는 선언 한 줄로 이 검사를 받는다(WP-9 `ExternalAgent`가 그 첫 사례다 — 검증 코드를 한 줄도 고치지 않고 합류했다). 실존(카탈로그 해소)은 파일시스템 소관이라 보지 않는다. 위임 지시 자리는 **이름을 지어내지 않는 대신 고치라는 표시**로 나간다(`compiler/emit/common.delegate_to_phrase`) |
...
| `undeclared_external_plugin` | **컴포넌트의** 외부 참조가 미선언 플러그인을 가리킴 — 배선이 안 나가 런타임에 못 찾는다 (WP-WR). 대상 판정은 `external_plugin_refs()` 하나이고 종류를 묻지 않는다 — 외부 정본을 선언하는 종류가 같은 술어로 합류한다. **매칭 정책은 하나다**(`config.plugin_ids_match`, 2026-09-19 리뷰로 단일화): 정확 일치 또는 **한쪽만 bare**일 때 bare 이름 일치(`alpha` ↔ `alpha@mkt` 일치, `alpha@mkt1` ↔ `alpha@mkt2` 불일치). 카탈로그가 선언은 `플러그인@마켓`, `agent_type`/`skill_ref`는 bare로 내는 비대칭 때문에 완화가 필요하고, 참조의 출처(`source`/`skills`)로 정책을 가르지 않는다 — 상세는 `agents.md` "외부 플러그인 스킬" 절 |
| `external_skill_ref_marketplace` | 에이전트 `config.skills`의 외부 스킬 참조에 `@마켓`이 붙어 있음(`beta@mkt:lint`) — CC는 마켓 없는 `beta:lint`로 찾으므로(실측 CC 2.1.278) 산출이 원문 그대로 나가면 런타임에 조용히 해소되지 않는다. 경고 메시지가 정규형(`normalize_external_skill_ref`)을 제시한다. 판정은 `config.external_skill_ref_has_marketplace`, 수집은 `config.external_skill_refs()`(WP-B 리뷰 반영) |
...
| `fork_agent_undeclared_plugin` | `플러그인:이름`의 플러그인이 `external_plugins`에 없으면 **에러** — 플러그인이 켜지지 않아 역시 조용히 범용으로 돈다(외부 에이전트의 `undeclared_external_plugin`은 경고지만 fork는 에러 — 이 비대칭을 유지하기로 한 근거는 `docs/backlog.md`). 그 에이전트가 플러그인에 실제로 있는지는 파일시스템이라 검증기가 보지 않는다(피커·MCP는 카탈로그로 거른다) |
