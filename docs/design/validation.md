...
| `undeclared_external_plugin` | **컴포넌트의** 외부 참조가 미선언 플러그인을 가리킴 — 배선이 안 나가 런타임에 못 찾는다 (WP-WR). 대상 판정은 `external_plugin_refs()` 하나이고 종류를 묻지 않는다 — 외부 정본을 선언하는 종류가 같은 술어로 합류한다 |
