...
| `external_source_missing` | **외부 정본을 선언한 컴포넌트**의 source 빈 값·형식 불일치 경고 (`플러그인[@마켓]:이름`). 대상은 `c.external_source is not None`인 것 전부다 — WP-2b에서 랩핑 전용 `wrapped_source_missing`을 **개명·일반화**했다(Q34). 종류를 묻지 않으므로 외부 정본을 갖는 새 종류는 선언 한 줄로 이 검사를 받는다(WP-9 `ExternalAgent`가 그 첫 사례다 — 검증 코드를 한 줄도 고치지 않고 합류했다). 실존(카탈로그 해소)은 파일시스템 소관이라 보지 않는다 |
