...
| `undeclared_external_plugin` | **컴포넌트의** 외부 참조가 미선언 플러그인을 가리킴 — 배선이 안 나가 런타임에 못 찾는다 (WP-WR). 대상 판정은 `external_plugin_refs()` 하나이고 종류를 묻지 않는다 — 랩핑 스킬과 외부 플러그인 에이전트(WP-9)가 같은 술어로 합류한다 |
...
| `fork_agent_undeclared_plugin` | `플러그인:이름`의 플러그인이 `external_plugins`에 없으면 **에러** — 플러그인이 켜지지 않아 역시 조용히 범용으로 돈다(랩핑·외부 에이전트의 `undeclared_external_plugin`은 경고지만 fork는 에러 — 이 비대칭을 유지하기로 한 근거는 `docs/backlog.md`). 그 에이전트가 플러그인에 실제로 있는지는 파일시스템이라 검증기가 보지 않는다(피커·MCP는 카탈로그로 거른다) |
...
**랩핑 전용으로 남은 자리는 오늘 5곳**이고 소스에 `# WRAPPED-ONLY` 태그가 붙어 있다 — 좁힘 4곳 + 문구 전용 1곳. 좁힘 4곳: `_agent_call_edges`의 caller (랩핑 스킬도 서브에이전트에서 돌지만 종전 집합에 없었고, 넓히면 `agent_chain_too_deep` 깊이와 `agent_calls_higher_model`이 조용히 달라진다), `mid_chain_user_invocable`의 대상, `wrapped_usage_conflict`의 용도 스위치 보유 판정, `_check_fork_agents`의 fork 스킬 게이트(랩핑 스킬도 `RUNS_IN_SUBAGENT`지만 `agent` 필드가 없어 fork 스킬 규칙의 대상이 아니다). 문구 전용 1곳: `wrapped_usage_conflict` 메시지가 읽는 `config.usage`. WrappedSkill 퇴역(WP-10)이 태그를 따라 전수 삭제한다 — 이 문장의 개수와 실제 태그 수가 어긋나면 `tests/model/test_wrapped_only_tags.py`가 실패한다.
