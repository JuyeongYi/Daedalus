...
| `undeclared_external_plugin` | **컴포넌트의** 외부 참조가 미선언 플러그인을 가리킴 — 배선이 안 나가 런타임에 못 찾는다 (WP-WR). 대상 판정은 `external_plugin_refs()` 하나이고 종류를 묻지 않는다 — 외부 정본을 선언하는 종류가 같은 술어로 합류한다. **매칭 정책은 참조 출처에 따라 둘로 갈린다**(WP-B, 2026-09-19): `ExternalAgent.source`처럼 마켓 유무를 사용자가 직접 고르는 참조는 설치 식별자 정확 일치(`alpha@mkt` ≠ `alpha`), 에이전트 `config.skills`의 외부 플러그인 스킬 참조(`플러그인:스킬` — CC가 fork 에이전트 `skills:`로 프리로드할 때 마켓을 쓰지 않는다, 실측)는 `external_plugin_id_declared`가 선언 쪽을 `partition("@")[0]`로도 맞춰 주는 **bare 완화**를 받는다. `naming.py`의 `_check_external_plugins`가 `comp.config.external_plugin_refs()`(bare)와 `comp.external_plugin_refs()`에서 그것을 뺀 나머지(정확)를 나눠 검사한다 — 상세는 `agents.md` "외부 플러그인 스킬" 절 |
