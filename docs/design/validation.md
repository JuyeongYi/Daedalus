...
| `dangling_string_reference` | `AgentConfigBase.skills`(에이전트 종류 전부), `PluginProject.reference_placements.skill_name`의 문자열 참조 실존 검사 (스킬 이름은 전역 skills 기준). fork 스킬 `agent`는 내장 이름·등록된 외부 fork 에이전트도 가리킬 수 있어 여기서 보지 않는다 — `fork_agent_missing`이 맡는다. **외부 플러그인 스킬 참조(`플러그인:스킬`, WP-B)는 여기서 보지 않는다** — `AgentConfigBase.name_refs(Bucket.SKILLS)`가 콜론 있는 참조를 제외하므로(`is_external_skill_ref`) 이 규칙은 프로젝트 스킬 이름만 본다. 그 참조의 배선 여부는 `undeclared_external_plugin`이 맡는다 |
...
| `undeclared_external_plugin` | **컴포넌트의** 외부 참조가 미선언 플러그인을 가리킴 — 배선이 안 나가 런타임에 못 찾는다 (WP-WR). 대상 판정은 `external_plugin_refs()` 하나이고 종류를 묻지 않는다 — 외부 정본을 선언하는 종류가 같은 술어로 합류한다. **매칭 정책은 하나다**(`config.plugin_ids_match`, 2026-09-19 리뷰로 단일화): 정확 일치 또는 **한쪽만 bare**일 때 bare 이름 일치(`alpha` ↔ `alpha@mkt` 일치, `alpha@mkt1` ↔ `alpha@mkt2` 불일치). 카탈로그가 선언은 `플러그인@마켓`, `agent_type`/`skill_ref`는 bare로 내는 비대칭 때문에 완화가 필요하고, 참조의 출처(`source`/`skills`)로 정책을 가르지 않는다 — 상세는 `agents.md` "외부 플러그인 스킬" 절 WP-EX에서 fork 쪽 에러 규칙(`fork_agent_undeclared_plugin`)이 사라져 **미선언은 이 한 갈래뿐**이다 |
...
| `unused_fork_agent` | 어떤 fork 스킬도 부르지 않는 **fork 실행 기반**(`IS_FORK_BASE` — `ForkAgent`·`ExternalForkAgent`) 경고 (WP-FK2/WP-EX) — `ForkAgent`는 `agents/<이름>.md`로 산출되고 `ExternalForkAgent`는 등록만 됐는데 아무도 실행하지 않는다(조용한 무동작 방지). 역참조 판정은 `model/plugin/placement.fork_skills_using` 한 곳이다(편집기 패널·삭제 확인·MCP `still_referenced_by`와 같은 실체). 워크플로 에이전트는 대상이 아니다 |
