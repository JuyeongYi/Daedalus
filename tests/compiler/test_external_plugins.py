...
짚는다. 외부 정본을 선언하는 종류는 그래프 노드 `ExternalAgent`(WP-9)와
fork 실행 기반 `ExternalForkAgent`(WP-EX), 그리고 에이전트 `config.skills`의
외부 플러그인 스킬 참조(WP-B, `플러그인:스킬`)다. 매칭 정책은 셋이 같다
(`plugin_ids_match` — 한쪽만 bare면 완화). 같은 source를 **두 번** 등록하면
