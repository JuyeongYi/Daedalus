...
**범위는 `daedalus/` 생산 소스뿐이다**(2026-09-19 명문화). `tests/`는 상한
밖이다 — 테스트 파일은 봉합선별로 자연히 길어지고, 상한을 걸면 "테스트를
쪼개려고" 픽스처를 공유 모듈로 빼는 압력이 생겨 오히려 읽기 어려워진다.
대신 800줄을 넘긴 테스트 파일은 `docs/backlog.md` §8에 분해 항목으로 남긴다
(오늘 해당: `tests/view/test_markdown_editor.py`).
"""
