...
| `WrappedSkill` | 다른 플러그인 스킬의 랩핑 | 본문 없음 — 정본은 외부 스킬(런타임 참조). 실행은 전용 서브에이전트가 맡는다 |
...
**외부 플러그인 스킬은 서브에이전트에서만 쓴다.** 랩핑 스킬은 SKILL.md(위임 지시)와 전용 실행 에이전트 `.md`(외부 스킬을 `skills` 프론트매터로 주입) 둘로 나온다 — 우리 워크플로를 모르는 외부 스킬의 지시가 메인 컨텍스트로 새지 않게 한다.
...
- [x] 외부 플러그인 스킬 랩핑(실행 서브에이전트 경유) · 에이전트 중첩 호출
