...
| `AgentDefinition` | 별도 컨텍스트의 작업자 | **내부 FSM 없음** — 절차는 본문, 결과 분기는 출력 포트, 다른 에이전트 호출은 호출 포트 |
...
`Validator.validate_project(project)` — 머신 수준 **18종** + 프로젝트 수준 **33종**(여기에 파일시스템을 봐야 판정되는 컴파일러 emit 경고 8종이 더 붙는다). 등급(에러/경고)의 단일 진실은 `validation/severity.py`의 `WARNING_RULES`다.

- **머신 수준**: 초기·최종 상태 포함성, 중첩 에이전트 금지, 전이 끝점 존재, 도달 불가 상태, Choice 완전성, 병렬 조인 수, 중복 배치, 출력 포트 필수성 등
- **프로젝트 수준**: 이름 중복·규약, 문자열 참조 실존(도구·훅·블랙보드·컴포넌트), 빌드 타깃 적합성, 진입점 의미론, 전이 스킬 재사용 금지, 에이전트 중첩 호출의 깊이·모델 티어, 작업 폴더 문서 규칙, 외부 플러그인 선언 정합 등
...
- **캔버스**: 드래그로 스킬·에이전트 배치, 포트 연결로 전이 생성, 경유점으로 엣지 정리, 우클릭으로 진입점 프리셋·미리보기·모델/effort·관련 경고. 유저가 `/스킬`로 시작할 수 있는 진입점은 테두리 색으로 구분된다
...
**거의 모든 기능이 MCP로 노출돼 있다 — 도구 85종.** 그래서 실제 사용감은 "도구를 호출한다"기보다 **옆자리에 앉은 사람과 같은 파일을 놓고 작업하는 쪽**에 가깝다.
...
- FSM 절차 서술, 다음 단계, 진입 맥락, 호출 계약, 위임, 출구 — 전부 **그래프에서 유도**한다
...
**외부 플러그인은 사용 선언으로 쓴다.** 선언하면 빌드가 dependencies(MARKETPLACE)/enabledPlugins(LOCAL)를 배선하고 그 플러그인의 스킬은 CC가 네이티브로 로드한다. 그 플러그인의 **서브에이전트**를 내 워크플로 노드로 쓰려면 외부 에이전트(`ExternalAgent`)를 만든다 — 우리는 파일을 만들지 않고 `플러그인:이름`으로 부른다.
