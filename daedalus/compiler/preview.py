...
**산출 게이트를 거치지 않는다.** 이번 빌드에 파일이 나가지 않는 컴포넌트도
미리보기는 정상 렌더된다. 미리보기의 질문은 "이 컴포넌트가 무엇으로 컴파일되는가"이지 "이번
...
def can_preview(component) -> bool:
    ...
    `emits_output()`이 **아니다**: 그것은 인스턴스 상태까지 보는 판정이라
    "이번 빌드에 나가는가"를 답한다. 진입점은 이 판정으로 액션을 비활성화한다.
    """
