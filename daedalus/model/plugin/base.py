...
class PluginComponent(ABC):
    ...
    `isinstance(c, ReferenceSkill)` 식으로 종류를 열거하면 새 종류가 생길 때마다
    ...
    **세 가지 기계적 규약** — 어기면 조용히 깨진다:
    ...
    def can_delete(self) -> tuple[bool, str | None]:
        """지울 수 있는가 — (가능 여부, 불가 **사유 절**).

        사유를 함께 돌려주는 이유는 거절이 조용하면 안 되기 때문이다(원칙 5):
        GUI는 이 문구를 비활성 툴팁으로, MCP는 거절 메시지로 쓴다.

        **사유는 완결 문장이 아니라 절이다** — "…이기 때문입니다" 꼴로, 판정
        ("삭제할 수 없습니다")과 대안 안내는 호출자가 소유한다. 모델이 문장을
        통째로 돌려주면 두 계층이 같은 말을 이어 붙여 문구가 겹친다.
        """
        return (True, None)
