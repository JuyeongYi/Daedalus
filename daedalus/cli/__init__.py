...
- :mod:`daedalus.cli.progress` — 진행 상태 파일(``state/__progress__.json``),
  같은 규약.
- :mod:`daedalus.cli.mcp_server` — 모델이 접하는 표면. stdio MCP 서버이고 도구
  7개가 코어 함수와 1:1이다. pyproject의 ``[project.scripts]``가
  ``daedalus-bb``를 여기로 건다.
- :mod:`daedalus.cli.blackboard` — 종전 argparse CLI. WP-BM 단계 3에서 폐기된다.
